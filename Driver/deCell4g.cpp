...
{                                                              // 检测ppp0网口
    ...
    {
        std::cout << "Socket create error" << std::endl;
        std::cout << "-------------ppp init socket fd = " << sock_fd << std::endl;
...
int deCell::pppInit(char *netName)
{
    ...
    while (retry--)
    {
        ...
        {
            // 如果检测失败，增加日志输出以便调试
            std::cout << "Failed to detect 4G connection on retry: " << 2 - retry << std::endl;
        }

        system("/home/root/g2020/mokuai/4g/ppp/ppp/quectel-ppp-kill");
        sleep(2); // 等待以确保 kill 命令完全终止 PPPD 进程
    }

    std::cout << "All retries failed. Exiting with error." << std::endl;
    return -1;
}
