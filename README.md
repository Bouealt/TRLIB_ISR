...
本项目是对原有系统进行的完全重构，从C语言过程式编程转向C++面向对象设计，主要改进包括：

### 架构优化
 - 重新设计为单Reactor多线程模型，替代原有的基于select的阻塞IO方式
 - 将原来的三个模块（ApManager、AppManager、NetManager）整合为统一的事件驱动架构
 - 采用工厂方法和依赖注入设计模式，提高代码可测试性和可维护性

### 性能提升
 - 使用epoll替代select，大幅提高并发连接处理能力
 - 引入线程池处理异步任务，避免主线程阻塞
 - 实现零拷贝数据传输，减少内存消耗和提高吞吐量

### 代码质量改进
 - 统一错误处理机制，提高系统稳定性
 - 采用智能指针管理资源，杜绝内存泄漏
 - 使用命名空间和更合理的类结构，提高代码可读性和组织性

### 新增特性
 - 实现智能通信选择器，基于多维度评分自动选择最佳通信接口
 - 增强的离线数据缓存机制，确保在网络不稳定环境下的数据完整性
 - 完善的断线重连和心跳检测机制，提高长期运行稳定性
...
| 编程范式 | C语言过程式 | C++面向对象 |
| 进程模型 | 三个独立进程 | 单进程多线程 |
| IO模型 | select阻塞IO | epoll事件驱动 |
| 资源管理 | 手动内存管理 | 智能指针RAII |
| 错误处理 | 打印错误日志 | 系统化的错误恢复 |
| 通信选择 | 静态配置 | 动态评分选择 |

### 原系统逻辑流程图图

![image](image/原始架构.png)

### 系统框架图

![image](image/系统框架.png)

## 事件处理流程

![image](image/详细处理流程.svg)
...
![image](image/私有协议结构.png)

## 私有协议主要通信流程

![image](image/私有协议通信流程.svg)
...
TRLIB智能网关系统已在多个实际项目中得到成功应用，展现了良好的稳定性和性能表现。

### 生态艇监测系统应用

TRLIB智能网关系统成功部署在水质监测生态艇上，为水环境治理提供了可靠的数据支持。生态艇作为移动式水质监测平台，能够在湖泊、河流等水域内灵活航行，实时采集水质数据。

![6001生态艇部署图](image/6001设备部署.png)

![9001生态艇部署图](image/9001设备部署.png)
