...
ISRConnection::ISRConnection(ISRServer *isrser, int clientFd, std::string name) : TcpConnection(isrser->getScheduler(), clientFd), mServer(isrser),
																				  mFd(clientFd),
																				  mName(name)
{
...
int ISRConnection::analysis01Mess()
{
	...
	if (std::string::npos == posStart || std::string::npos == posEnd)
		return 0; // 查找该包是否完整,npos表示没找到
