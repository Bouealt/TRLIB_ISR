...
std::unique_ptr<TRServer> TRServer::createNew(std::vector<InetAddress> serverAddrs)
{
	return std::unique_ptr<TRServer>(new TRServer(serverAddrs));
}

TRServer::TRServer(std::vector<InetAddress> serverAddrs) : mServerAddrs(serverAddrs)
{
	wanInit();
	pppInit();
