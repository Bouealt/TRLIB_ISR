...
std::unique_ptr<ISRServer> ISRServer::createNew(EventScheduler *scheduler, ThreadPool *threadPool, InetAddress &wifiAddr, InetAddress &lanAddr, std::vector<InetAddress> serverAddrs)
{
	return std::unique_ptr<ISRServer>(new ISRServer(scheduler, threadPool, wifiAddr, lanAddr, serverAddrs));
}

ISRServer::ISRServer(EventScheduler *scheduler, ThreadPool *threadPool, InetAddress &wifiAddr, InetAddress &lanAddr, std::vector<InetAddress> serverAddrs) : mDevice(Device::createNew(wifiAddr, lanAddr)),
...
void ISRServer::handleCloseConnect()
{
	...
	for (auto it : mDisConnectList)
	{
		...
		if (mConnectMap.find(fd) != mConnectMap.end())
		{
			...
			delete conn;
			std::cout<<"delet conn over"<<std::endl;
			mConnectMap.erase(fd);
		}
	}

	mDisConnectList.clear();
	std::cout<<"finish dis connect"<<std::endl;
}

void ISRServer::deviceConnectInit(std::unique_ptr<Device>& device)
{
...
void ISRServer::serverConnectInit(std::unique_ptr<TRServer>& server)
{
	...
	{ // 服务器连接map同步
		if (mServerMap.find(it.first) != mServerMap.end())
			continue;
...
{
	ISRServer *isrser = (ISRServer *)arg;
	// std::cout<<" ppp init timer"<<std::endl;
