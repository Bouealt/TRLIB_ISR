...
std::unique_ptr<Device> Device::createNew(InetAddress &wifiAddress, InetAddress &lanAddress)
{
    return std::unique_ptr<Device>(new Device(wifiAddress, lanAddress));
}

Device::Device(InetAddress &wifiAddress, InetAddress &lanAddress) : mWifiAddress(wifiAddress),
...
{
    if (-1 != mLoraFd)
        sockets::close(mLoraFd);
    if (-1 != mWifiFd)
        sockets::close(mWifiFd);
    if (-1 != mBlueToothFd)
        sockets::close(mBlueToothFd);
    if (-1 != mLanFd)
        sockets::close(mLanFd);
}
