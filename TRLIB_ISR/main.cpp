...
InetAddress aliyunAddr("120.78.234.51", 8888, "Aliyun");		// 阿里云服务器地址g2020.top
...
int main()
{
	...
	std::unique_ptr<ISRServer> isrServer = ISRServer::createNew(scheduler, threadPool, isrWifiAddr, isrLanAddr, serverAddrs);
	isrServer->start();
	scheduler->loop(); // 处理事件
	return 0;
}